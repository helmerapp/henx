...
func encoderIngestYuvFrame(
  ...
  if enc.assetWriterInput.isReadyForMoreMediaData {
    ...
    if !success {
      print("AssetWriter: \(enc.assetWriter.error?.localizedDescription ?? "Unknown error")")
    }
  } else {
    print("AssetWriter: not ready for more data")
  }
}
...
func encoderFinish(_ enc: Encoder) {
  ...
  while enc.assetWriter.status == .writing {
    print("AssetWriter: still writing...")
    usleep(500000)
  }

  print("AssetWriter: finished writing!")
}
